...
type Panel = { i: number; r: number; fx: number; fy: number };
...
const PANELS: Panel[] = (() => {
  ...
  for (let r = 0; r < ROWS; r++) {
    for (let c = 0; c < COLS; c++) {
      ...
      out.push({ i, r, fx: ARRAY_X + c * P_W, fy: rowTop(r) });
      i++;
    }
  }
  return out;
...
export const ARRAY_AREA = Math.round(ARRAY_FW * ARRAY_FH); // ≈ ১০,৮০০
export const EQ_AREA = EQ.fw * EQ.fh; // ৩৮৪
export const ROOF_AREA = ROOF_W * ROOF_H; // ১৬,০০০
export const USED_AREA = ARRAY_AREA + EQ_AREA;
export const FREE_AREA = ROOF_AREA - USED_AREA;
export const USED_PCT = Math.round((USED_AREA / ROOF_AREA) * 100);
...
export function RoofLayoutSection() {
  ...
  return (
    <section aria-labelledby="roof-layout" className="print-break">
      <style>{`
        @media print {
      ...
      >
        <LayoutGrid className="h-5 w-5 text-brand-red" />
        ছাদে কী কোথায় বসবে — ধাপে ধাপে
      </h2>
      <p className="mt-3 text-sm text-ink-soft">
        প্যানেল, মাউন্টিং স্ট্রাকচার, ইনভার্টার, কম্বাইনার বক্স, ক্যাবল রুট, আর্থিং ও
        হাঁটার পথ — প্রতিটি কোথায় বসবে এবং কতটুকু জায়গা নেবে তা নিচের ড্রয়িংয়ে দেখানো
        হয়েছে। “চালান” চাপলে ধাপে ধাপে বসতে দেখবেন।
      </p>

      {/* ধাপের বিবরণ */}
      <div className="mt-4 rounded-lg border border-brand-red/20 border-l-4 border-l-brand-red bg-brand-red-soft p-4">
        <p className="text-sm font-bold text-brand-red">{STAGES[stage].t}</p>
        <p className="mt-1 text-sm text-ink-soft">{STAGES[stage].d}</p>
      </div>

      {/* ড্রয়িং */}
      <div className="mt-4 overflow-x-auto rounded-lg border border-brand-red/20 bg-[#fbf9f5] p-2">
        <svg
      ...
      {/* লেজেন্ড */}
      <div className="mt-4 rounded-lg border border-brand-red/20 bg-card p-4">
      ...
      {/* জায়গার হিসাব */}
      <div className="mt-4 overflow-x-auto rounded-lg border border-brand-red/20">
          ...
          <tbody>
            {(
              [
                ...
                ["ইনভার্টার ও সুইচগিয়ার জোন", `${bn(EQ.fw)} × ${bn(EQ.fh)} ফুট`, EQ_AREA],
      ...
      <p className="mt-3 text-sm text-ink-soft">
        শুধু প্যানেলগুলোর নিজস্ব আয়তন {bnNum(PANEL_AREA)} বর্গফুট; সারির মাঝে ছায়া এড়ানোর
        ফাঁক ও হাঁটার পথ যোগ করলে অ্যারে ব্লক দাঁড়ায় {bnNum(ARRAY_AREA)} বর্গফুট।
      </p>

      {/* সিদ্ধান্ত */}
      <div className="mt-4 rounded-lg border-2 border-brand-red bg-brand-red-soft p-5">
        ...
        <ul className="mt-3 list-disc space-y-1.5 pl-5 text-sm text-ink-soft">
          <li>
            প্যানেল অ্যারে + যন্ত্রপাতি জোন মিলে লাগছে {bnNum(USED_AREA)} বর্গফুট — ছাদের{" "}
            {bn(USED_PCT)}%।
          </li>
          <li>
          ...
          <li>
            শর্ত: ছাদ ছায়ামুক্ত হতে হবে (পানির ট্যাংক, লিফট রুম, পাশের বিল্ডিং, গাছ) এবং
            ছাদ প্রতি বর্গফুটে অতিরিক্ত ৪–৫ কেজি ভার নিতে সক্ষম হতে হবে।
          </li>
          <li>ছাদের প্রকৃত আকার ও বাধা অনুযায়ী সারি বিন্যাস সাইট সার্ভেতে চূড়ান্ত হবে।</li>
        </ul>
      </div>
    </section>
  );
